  ...
  return (
    <div className='w-[90%]  flex flex-col gap-2.5 bg-white items-center shadow-2xl shadow-[#00000058] rounded-2xl pb-5'>
      ...
      <div className='w-full h-20 flex justify-between items-center px-2.5 '>
        ...
        <div className='flex justify-center items-center gap-2.5 md:gap-5' onClick={()=>navigate(`/profile/${post.author.userName}`)}>
        ...
        {userData._id != post.author._id && <FollowButtton tailwind={'px-2.5 min-w-15  w- md:min-w-25 py-1.25 h-7.5 md:h-10 bg-black text-white rounded-2xl text-[14px] md:text-[16px]'} targetUserId={post.author._id} />}
      ...
      <div className='w-full h-15 flex justify-between items-center px-5 mt-2.5 '>
        ...
        <div onClick={handleSaved}>
          {!userData.saved.includes(post?._id) && <FaRegBookmark />}
          {userData.saved.includes(post?._id) && <FaBookmark />}
      ...
      {showComment &&
        <div className='w-full flex flex-col gap-7.5 pb-5'>
          <div className='w-full h-20 flex items-center justify-between px-5 relative'>
            ...
            {message && <button className='absolute right-5 cursor-pointer' onClick={handleComment}><IoSendSharp className='w-6.25 h-6.25' /></button> }
            
          </div>

          <div className='w-full max-h-75 overflow-auto'>
            {post.comments.map((com, index) => (
              <div key={index} className='w-full px-5 py-5 flex items-center gap-5 border-b-2 border-be-gray-200'>
                <div className='w-10 h-10 md:w-15 md:h-15 border-2 border-black rounded-full cursor-pointer overflow-hidden'>
