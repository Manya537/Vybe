...
const Messages = () => {
  ...
  return (
    <div className='w-full min-h-screen flex flex-col bg-black gap-5 p-2.5'>
         <div  className='w-full h-20 flex  items-center gap-5 px-5'>
                    <IoIosArrowRoundBack className='text-white cursor-pointer lg:hidden w-6.25 h-6.25' onClick={()=>navigate(`/`)}/>
  ...
  onlineUsers?.includes(user._id) && (
    <OnlineUser key={index} user={user}/>
      ...
      {onlineUsers?.includes(user._id) && <div className='text-blue-800 text-[15px]'>Active Now</div>}
    </div>
            </div>
    ))}
